...
export default function AdminThreshold({ tipo, titolo, unita }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      alert(e.message || "Errore salvataggio soglie");
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card-white rounded-2xl p-4 space-y-3">
        <div>
          <div className="text-xs text-gray-600 font-semibold mb-2">Soglia minima ({unita})</div>
          <input
        ...
        <div>
          <div className="text-xs text-gray-600 font-semibold mb-2">Soglia massima ({unita})</div>
          <input
            className="input-honey w-full h-10 rounded-xl px-3 text-sm text-gray-900"
            type="number"
            value={max}
