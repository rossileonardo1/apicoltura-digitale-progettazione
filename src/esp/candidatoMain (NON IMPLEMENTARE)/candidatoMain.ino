...
void initWiFi() {
  ...
  // Configura reti con WiFiMulti
  wifiMulti.addAP(WIFI_SSID, WIFI_PASSWORD);
  wifiMulti.addAP("Gruppo4Network", "Networks");  // Backup hotspot
  
  Serial.println("  + WiFiMulti configurato (2 reti)");
}
...
// ============================================================================
// RICONNESSIONE WIFI - Da chiamare nel loop
// ============================================================================
void riconnessioneWiFi() {
  ...
  if (wifiMulti.run() != WL_CONNECTED) {
    Serial.println("\n[WIFI] !  Disconnesso, riconnessione...");
    wifiConnesso = false;
    
    // Tenta riconnessione (WiFiMulti prova tutte le reti)
    if (wifiMulti.run() == WL_CONNECTED) {
      wifiConnesso = true;
      Serial. print("[WIFI] + Riconnesso a:  ");
      ...
      Serial.println("[WIFI] ! Riconnessione fallita, riprovo tra 10s");
    }
  } else {
    wifiConnesso = true;
  }
...
void setup() {
  ...
  Serial.println("  Main Controller v2.3");
  Serial.println("========================================");
  Serial.println();
...
void loop() {
  ...
  // ──────────────────────────────────────
  // RICONNESSIONE WIFI (ogni 10 secondi)
  // ──────────────────────────────────────
  riconnessioneWiFi();
